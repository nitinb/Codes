...
int main(){
    ...
    for(int i = 0; i < num_test_cases; i++){
       ...
       while(1){
           ...
           std::cout << "size of heap "<< heap_ready_pratas.size() << std::endl;           
           heap_ready_pratas.push(ready_cook);           
       }
       std::cout << ready_cook->num_prata*(ready_cook->num_prata+1)/2 << std::endl;
       std::cout << "prata by last-cook "<< ready_cook->num_prata << std::endl;
    }
    while(1){ continue; }
}
