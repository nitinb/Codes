...
* Last Modified : Monday 04 July 2011 03:42:19 PM IST
...
std::string calculate_sum_pdt(std::vector<std::string> partial_mult){
   std::string output;
   int _size = partial_mult.size();
   while(_size > 1){
//      std::cout << "size (decr): " << partial_mult.size() << std::endl;
      output = calculate_sum(partial_mult[_size-2], partial_mult[_size-1]);
      partial_mult[_size-2] = output;
      _size--;
   }
