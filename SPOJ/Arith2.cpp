...
//       std::cout << num1 << "  " << op << "  "  << num2 << std::endl;
       ...
       _ret = "";
       if(op == '+'){ 
          _ret = calculate_sum(num1, num2);
       }
       else if(op == '-'){ 
          _ret = calculate_diff(num1, num2);
          _max = max(num1.length(), num2.length()+1, _ret.length());          
       }
       else if(op == '*') { 
          ...
          if(op == '*' && partial_mult.size() > 1){
             print_del( max(num1.length(), num2.length()+1, 0) );
